...
sp_taille_poids <- read_excel("sp_taille_poids.xlsx")  # excel essentiel a avoir dans le dossier de l'app. On le charge ici. COmprend les tailles et poids maximaux des especes. Permet d'enlever les grosses aberrations
cat(">>> sp_taille_poids chargé\n")  # sert a verifier que ce code tourne bien quanq l'app est RUN et que le document taille_poids est bien chargé

tableau_a_b_especes <- read_csv("tableau_a_b_especes.csv") # excel essentiel a avoir dans le dossier de l'app. On le charge ici, il comprend les coeeficient a et b des 3 sources : Fishbase, SIH et Obsbio de l'Ifremer

banyuls_shp <- st_read("carto/delimitation.shp")%>% # pour banyuls
...
calanque_shp <- st_read("carto/PNCal_Zones_Peche_Mer_L93.shp") %>%
