...
ui <- page_navbar(
  ...
  nav_panel("Informations complémentaires",                                     # page 2
            ...
            layout_columns(
              ...
              div(
                ...
                selectInput("annee_page2", "Année (concerne toute la page)",
                            choices = "Toutes les années", width = "220px")
              ),
              div(
                class = "dashboard-card",
  ...
  nav_panel("Cartographie", # onglet 3/page 3 : disponible pour les AMP avec des sous zones de pêche 
            ...
            # Bloc "Année" en haut, pleine largeur
            layout_columns(
              col_widths = c(3),
              fill = FALSE,
              ...
              div(
                class = "dashboard-card",
                selectInput("annee_page3", "Année (concerne toute la page)",
                            choices = "Toutes les années", width = "220px")
              )
            ),
            
            # Cartes en dessous, chacune pleine largeur
            layout_columns(
              col_widths = c(12),
...
server <- function(input, output, session) {
  ...
  observeEvent(df(), {
    ...
    updateSelectInput(session, "annee_page2", choices = choix, selected = "Toutes les années")
    updateSelectInput(session, "annee_page3", choices = choix, selected = "Toutes les années")
  ...
  # -------------------------------------------------------------------
  # Jeu de données de la page 1, filtré par l'année choisie (input$annee_page1)
  # -------------------------------------------------------------------
  # Ce réactif centralise le filtrage année pour TOUTE la page 1 : il est
  # utilisé par le tableau des espèces, les CPUE (3 onglets) et la biomasse,
  # pour que le sélecteur "Année" en haut de la page 1 s'applique bien à
  # l'intégralité des éléments de la page, et pas seulement à certains blocs.
  # -------------------------------------------------------------------
  data_page1 <- reactive({
    req(data_avec_poids())
  ...
  liste_especes <- reactive({
    req(data_page1())                                                           # On s'assure que `data_page1()` (poids_final_total, filtré par année page 1) est disponible avant de continuer.
    data_page1() %>%
  ...
  df_top10 <- reactive({
    ...
    data_page1() %>%                                                           # data_page1() est déjà filtrée par l'année choisie sur la page 1.
  ...
  cpue_espece_mode <- reactive({
    req(data_page1(), cpue_espece_top10(), input$mode_peche_cpue)
  ...
  cpue_espece_mode_tableau <- reactive({
    req(data_page1(), cpue_espece_top10(), input$mode_peche_cpue_tableau)
  ...
  data_taille_espece <- reactive({
    ...
    filtrer_annee(df(), input$annee_page2) %>%
  ...
  # ---------------------------------------------------------------------
  # Taille minimale de capture (TMC) pour l'espèce sélectionnée
  # ---------------------------------------------------------------------
  # On récupère la valeur TMC_Med correspondant à l'espèce choisie
  # dans sp_taille_poids, via NomVernaculaire.
  # ---------------------------------------------------------------------
  
  tmc_espece <- reactive({
    req(input$espece_page2)
  ...
  output$plot_taille_espece <- renderPlot({
    ...
    q95  <- quantile(data_taille_espece()$Taille_MM, probs = 0.995, na.rm = TRUE)  # changer ici la limite haute (qui permet de ne pas afficher les valeurs extremes qui sotn aberrantes)
    xmax <- ceiling(q95 / 10) * 10
    
    tmc <- tmc_espece()
    tmc_val <- if (length(tmc) > 0 && !is.na(tmc[1])) tmc[1] else NA
    
    p <- ggplot(data_taille_espece(), aes(x = Taille_MM)) +
      geom_histogram(
        binwidth = 10,
        fill = "lightblue",
        color = "black",
    ...
    # Ajout du trait rouge uniquement si une TMC existe pour l'espèce
    if (!is.na(tmc_val)) {
      p <- p +
        geom_vline(
          aes(xintercept = tmc_val, color = "Taille minimale de capture Méditerranée"),
          linetype = "dashed",
          linewidth = 1
        ) +
        scale_color_manual(name = NULL, values = c("Taille minimale de capture Méditerranée" = "red"))
    }
  ...
  rtp_espece_data <- reactive({
    ...
    data_coherente_FB <- filtrer_annee(df(), input$annee_page2) %>%
      filter(NomVernaculaire == input$espece_page2) %>%
  ...
  shp_pression_espece <- reactive({
    ...
    data_sp <- filtrer_annee(df(), input$annee_page3) %>%
  ...
  shp_pression_globale <- reactive({
    ...
    captures_par_zone <- filtrer_annee(df(), input$annee_page3) %>%
      group_by(Nom_SousZoneDePeche) %>%
      summarise(
        total_nb_captures = sum(Nb_de_prises, na.rm = TRUE),
        nb_sorties        = n(),
